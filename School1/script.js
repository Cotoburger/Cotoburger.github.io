...
document.addEventListener("DOMContentLoaded", () => {
    ...
    menuIcon.addEventListener("mousedown", (event) => {
        ...
        if (navigator.vibrate) {
            navigator.vibrate([50]);  // Вибрация 50ms, пауза 20ms, снова 50ms, и так далее
        }
    });
    ...
    links.forEach(link => {
        link.addEventListener("click", (event) => {
            ...
            if (navigator.vibrate) {
                navigator.vibrate(50); // Вибрация длительностью 50 миллисекунд
            }
        });
    });
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const maxSnowflakes = 150; // Максимальное количество снежинок на экране
    ...
    function snowflakesLoop() {
        ...
        }, 125); // Увеличенный интервал между созданием снежинок
    }
